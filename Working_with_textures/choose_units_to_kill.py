...
def choose_units_to_kill(window, fullscreen, units):
    ...
    if len(units) > 4:
        buttons = draw_some_buttons(window, 4,
                                    ...
                                    (0, 0, 100, 200), 255, 215, 0, 85, 107, 47, 10)  # отступ между ними
        names = []
        for i in units[4:]:
            names += [i.base.name]
        buttons += draw_some_buttons(window, len(names), names,
                                     (700, 0, 100, 150), 255, 215, 0, 85, 107, 47, 10)  # отступ между ними
    else:
        names = []
        for i in units:
            names += [i.base.name]
    ...
    buttons += draw_some_buttons(window, 1, ["End"],
                                     (700, 150, 100, 75), 255, 215, 0, 85, 107, 47, 15)  # отступ между ними
    pygame.display.update()
    return buttons
